...
def pyspark_output(spark, df, write_options_dict, param_dict):
    ...
    p_version_schema_tango = int("{{{Transf_PRY_CargaInicial.P_VERSION_SCHEMA_TANGO}}}")
    kafka_auth = "{{{Transf_PRY_PaymentHub.P_KAFKA_AUTH}}}"
    usr_match = re.search(r"username=['\"]([^'\"]+)['\"]", kafka_auth)
    pwd_match = re.search(r"password=['\"]([^'\"]+)['\"]", kafka_auth)
    ...
    if "schema_tango_cache" not in globals():
        ...
        schema_tango_cache = sr_client_driver.get_version(
            f"{topic_tango}-value", p_version_schema_tango
        ).schema.schema_str
    ...
    def send_partition(partition):
        ...
        p = Producer(
            {
                ...
                "client.id": f"pyspark-saldos-producer-{uuid.uuid4()}",
        ...
        for row in partition:
            ...
            cuenta_id = (
                record.get("data", {})
                .get("identifications", [{}])[0]
                .get("identifier", {})
                .get("value", "")
            )
            message_key = str(cuenta_id) if cuenta_id else str(uuid.uuid4())

            value_bytes_tango = avro_serializer_tango(record, ctx_tango)
            p.produce(
                topic=topic_tango,
