def pyspark_output(spark, df, write_options_dict, param_dict):
    ...
    s3_path = "{{{BUCKET_S3_DL_002_NRT}}}/data/pro/bp/acp/pas/{{{P_NOMBRE_TABLA}}}"

    df_write = df.filter(
        (F.col("numero_comprobante_reposting").isNull())
        | (F.col("numero_comprobante_reposting") == "")
        | (F.col("tipo_procesamiento").isNull())
        | (F.col("tipo_procesamiento") == "")
    ).drop("reposting_completo")

    df_write.persist()

    df_write.write.format("delta").option("compression", "snappy").option(
        "mergeSchema", "true"
    ).mode("append").save(s3_path)
    ...
    def delete_udf(row):
        ...
        try:
            delete_query = """
                DELETE FROM pas.movimientos_cuenta
                WHERE periodo = %s
